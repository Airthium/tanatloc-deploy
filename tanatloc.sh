...
help() {
	...
	echo -e " - [${Cyan}database${Off}, ${Cyan}data${Off}] ${Purple}remove${Off}"
	echo -e "   Remove database or data docker"
...
if [ $# -eq 0 ]; then
	...
	### Database
		...
		#### Restore
			...
			checkValue "$value"
			docker-compose <"$value" run -e PGPASSWORD="password" database psql -h database -U postgres

		#### Remove
		elif [ "$option" = "remove" ]; then
			read -rp "Do you want to remove the database? (y/N) " yn
			if [ "$yn" = "y" ]; then
				docker-compose run -e PGPASSWORD="password" database psql -h database -U postgres -c "DROP DATABASE tanatloc2"
			fi
	...
	### Data
		...
		#### Remove
			...
			id=$(docker ps -aqf "name=tanatloc-deploy_tanatlocData")
			docker stop "$id"
			docker rm "$id"
